...
# Add new function to initialize swarm
init_swarm() {
...
createnewdocker() {
    ...
    # Create config files
    docker config create $user-config - <<EOF
NAMA_PELANGGAN=$user
...
EOF
    ...
    docker service create \
        --name c9-$user \
        --config source=$user-config,target=/config/.env \
        --publish published=$port,target=8080 \
        --mount type=volume,source=${user}_data,target=/home/workspace \
        $image

    echo "Service c9-$user created"
}
...
createnewdockermemlimit() {
    ...
    echo
    echo "Creating docker service with resource limits:"
    echo "Username: $user"
    echo "Password: $pw"
    echo "Port: $port"
    echo "Memory Limit: $limit"
...
EOF
    ...
    # Create service with configs and resource limits
    docker service create \
        --name c9-$user \
        --config source=$user-config,target=/config/.env \
        --publish published=$port,target=8080 \
    ...
    echo "Service c9-$user created with resource limits"
}
...
stopdocker() {
    ...
    *)
    ...
    docker service scale c9-$user=0
}
...
startdocker() {
    ...
    *)
    ...
    docker service scale c9-$user=1
}
...
deletedocker() {
    ...
    *)
    ...
    docker service rm c9-$user
    docker config rm $user-config
    docker volume rm ${user}_data
}
...
restartdocker() {
    ...
    if echo "$containers" | grep -qw "$container_name"; then
        docker service update --force c9-$user
...
restore_manual() {
    ...
    if [[ -z "$username" || -z "$drive_name" || -z "$folder" ]]; then
        echo "Usage: c9cli restore -u USERNAME -n DRIVE_NAME -f FOLDER [-d DATE]"
        return 1
    fi

    echo "Starting manual restore for $username..."

    # Get list of available backups
    backups=$(rclone lsf "$drive_name:$folder" --include "*-$username-*.zip" | sort -r)
    ...
    # If date not specified, show available backups
    if [[ -z "$date" ]]; then
        ...
        backup_file=$(echo "$backups" | grep "$date")
    fi
    ...
    # Download backup
    if ! rclone copy "$drive_name:$folder/$backup_file" "$temp_dir/"; then
    ...
    service_name="c9-$username"
    if ! docker service inspect "$service_name" >/dev/null 2>&1; then
        echo "Service $service_name not found"
        read -p "Enter port for new service: " port
        read -p "Enter memory limit (e.g. 1024m): " memory
        read -p "Enter CPU limit (e.g. 1.0): " cpu_limit

        # Create new service
        docker service create \
            --name "$service_name" \
            --mount type=bind,source="$temp_dir/workspace",target=/workspace \
            --publish published="$port",target=8080 \
            --limit-memory "$memory" \
            --limit-cpu "$cpu_limit" \
            "gvoze32/cloud9:jammy"
    else
        # Update existing service
        docker service update \
            --force \
            --mount-add type=bind,source="$temp_dir/workspace",target=/workspace \
            "$service_name"
    fi

    if [ $? -eq 0 ]; then
        echo "Restore completed successfully"
        # Keep backup for 1 hour before cleanup
        (
            sleep 3600
            rm -rf "$temp_dir"
        ) &
